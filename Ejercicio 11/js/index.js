...
// columnas (td)
// createElemnt ->
//append -> anexar un componente dentro de un html 
// =>  -->

// addEventListener -> añadir un escuchador de eventos  - ejemplo quiero que haga click 
...
/*for(const index in products){ // recorro el arreglo ( in products )
    const tr = document.createElement("tr")
    for(const key in products[index]){  // productos [index] -> recorro el objeto,. Si quiero especificar dentro del objeto products[index].name  
        const td = document.createElement("td")
    }
    table.append(tr)            // tr - padre , td - hijo ---> tr , anexa con el append al td para que pueda leerlo
}     */               
